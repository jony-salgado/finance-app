...
async def create_link_token():
    ...
    try:
        ...
        return {
            "linkToken": response.access_token
        }
    except Exception as e:
...
async def perform_sync_for_item(item_id: str) -> dict:
    ...
    print(f"Sync completed successfully for item {item_id}. Added {transactions_added} transactions.")
    return {
        "status": "success",
...
async def periodic_pluggy_sync():
    ...
    while True:
        try:
            ...
            res = supabase.table("accounts").select("provider_item_id").eq("provider_id", PROVIDER_ID_PLUGGY).execute()
            if res.data:
                # Get unique item ids
                item_ids = {item["provider_item_id"] for item in res.data if item.get("provider_item_id")}
