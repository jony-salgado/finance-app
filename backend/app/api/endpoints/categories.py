...
def update_category(category_id: str, category: CategoryCreate) -> dict:
    """
    ...
    try:
        db_data = category.model_dump(mode="json", exclude_none=True, by_alias=False)
        response = (
            supabase.table("categories").update(db_data).eq("id", category_id).execute()
        )

        if not response.data:
            raise HTTPException(
                status_code=404, detail="Category not found or could not be updated"
            )
