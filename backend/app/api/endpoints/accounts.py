...
def create_account(account: AccountCreate) -> dict:
    ...
    try:
        ...
        if not response.data:
            raise HTTPException(
                status_code=400, detail="Error creating account: No data returned."
            )
...
def update_account(account_id: str, account: AccountCreate) -> dict:
    """
    ...
    try:
        db_data = account.model_dump(mode="json", exclude_none=True, by_alias=False)
        response = (
            supabase.table("accounts").update(db_data).eq("id", account_id).execute()
        )

        if not response.data:
            raise HTTPException(
                status_code=404, detail="Account not found or could not be updated"
            )
        ...
        if "violates row-level security policy" in error_msg:
            raise HTTPException(
                status_code=403,
                detail="Permission denied: Row Level Security policy prevents updating.",
            )
        raise HTTPException(status_code=500, detail=f"Database error: {error_msg}")
