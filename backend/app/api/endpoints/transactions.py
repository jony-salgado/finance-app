...
def create_transaction(transaction: TransactionCreate) -> dict:
    ...
    try:
        ...
        if not response.data:
            raise HTTPException(
                status_code=400,
                detail="Error creating transaction: No data returned from database.",
            )
...
@router.put(
    "/{transaction_id}", response_model=Transaction, response_model_by_alias=True
)
def update_transaction(transaction_id: str, transaction: TransactionCreate) -> dict:
    """
