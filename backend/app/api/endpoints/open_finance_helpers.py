...
def map_pluggy_account_type(p_account) -> str:
    ...
    if p_account.type == PLUGGY_TYPE_CREDIT or getattr(p_account, "subtype", None) == PLUGGY_SUBTYPE_CREDIT_CARD:
        p_local_type = ACCOUNT_TYPE_CREDIT_CARD
    elif p_account.type == PLUGGY_TYPE_INVESTMENT or getattr(p_account, "subtype", None) == PLUGGY_SUBTYPE_INVESTMENT_ACCOUNT:
        p_local_type = ACCOUNT_TYPE_INVESTMENT
    return p_local_type

def match_credit_card_by_digits(p_account, existing_accounts: List[dict], matched_ids: set, local_type: str) -> Optional[dict]:
    """
    ...
    for acc in existing_accounts:
        if acc["id"] in matched_ids:
            continue
        if acc.get("type") == ACCOUNT_TYPE_CREDIT_CARD and acc.get("card_last_digits") == last_digits:
            return acc
    return None

def match_by_provider_id(p_account, existing_accounts: List[dict], matched_ids: set) -> Optional[dict]:
    """
...
def match_by_name_similarity(p_account, existing_accounts: List[dict], matched_ids: set, local_type: str) -> Optional[dict]:
    """
    ...
    for acc in existing_accounts:
        ...
        for brand in SUPPORTED_BRANDS:
            if brand in acc_name_norm and (brand in p_name_norm or (brand == BRAND_NUBANK and BRAND_NU_PAGAMENTOS in p_name_norm)):
                if acc.get("type") == local_type:
                    return acc
    return None

def find_matching_account(p_account, existing_accounts: List[dict], matched_ids: Optional[set] = None) -> Optional[dict]:
    """
    ...
    matched = match_credit_card_by_digits(p_account, existing_accounts, matched_ids, local_type)
    if matched:
        return matched
    ...
    matched = match_by_name_similarity(p_account, existing_accounts, matched_ids, local_type)
    if matched:
        return matched

    return None

def generate_unique_account_name(p_account, local_type: str, all_existing_accounts: List[dict]) -> str:
    """
    Generates a unique name for the new account, appending suffix if name exists
    """
    base_name = p_account.name
    if local_type == ACCOUNT_TYPE_CREDIT_CARD and KEYWORD_CREDITO not in base_name.lower() and KEYWORD_CARTAO not in base_name.lower():
        base_name = f"{base_name} {SUFFIX_CREDITO}"
    elif local_type == ACCOUNT_TYPE_CHECKING and KEYWORD_CORRENTE not in base_name.lower() and KEYWORD_CONTA not in base_name.lower():
        base_name = f"{base_name} {SUFFIX_CORRENTE}"
    elif local_type == ACCOUNT_TYPE_INVESTMENT and KEYWORD_INVESTIMENTO not in base_name.lower():
        base_name = f"{base_name} {SUFFIX_INVESTIMENTO}"

    # Check if this name already exists in the database
    name_exists = any(acc["name"].lower() == base_name.lower() for acc in all_existing_accounts)
    if name_exists:
        digits = "".join(c for c in p_account.number if c.isdigit()) if p_account.number else ""
...
def prepare_existing_account_data(p_account, existing_record: dict, local_type: str, item_id: str) -> dict:
    """
    ...
    if local_type == ACCOUNT_TYPE_CREDIT_CARD:
        if not existing_record.get("card_last_digits"):
            account_data["card_last_digits"] = p_account.number[-4:] if p_account.number else DEFAULT_CARD_LAST_DIGITS
        if not existing_record.get("card_color"):
            account_data["card_color"] = DEFAULT_CARD_COLOR
    return account_data

def prepare_new_account_data(p_account, all_existing_accounts: List[dict], local_type: str, item_id: str) -> dict:
    """
    Prepares account data dictionary for a new account to be created in Supabase
    """
    base_name = generate_unique_account_name(p_account, local_type, all_existing_accounts)
    ...
    if local_type == ACCOUNT_TYPE_CREDIT_CARD:
        ...
        account_data["closing_day"] = due_day - DEFAULT_CLOSING_DAY_OFFSET if due_day > DEFAULT_CLOSING_DAY_OFFSET else DEFAULT_CLOSING_DAY_FALLBACK
        account_data["card_last_digits"] = p_account.number[-4:] if p_account.number else DEFAULT_CARD_LAST_DIGITS
        account_data["card_color"] = DEFAULT_CARD_COLOR
    return account_data
...
def fetch_pluggy_transactions(account_id: str, from_date: datetime.datetime, api_key: str) -> list:
    """
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"Error fetching transactions from Pluggy API: {res.status_code} {res.text}")
            return []
        data = res.json()
...
def sync_single_account(p_account, all_existing_accounts: List[dict], matched_ids: set, item_id: str) -> Optional[dict]:
    """
    ...
    """
    existing_record = find_matching_account(p_account, all_existing_accounts, matched_ids)
    local_type = map_pluggy_account_type(p_account)
    is_new = False

    if existing_record:
        matched_ids.add(existing_record["id"])
        account_data = prepare_existing_account_data(p_account, existing_record, local_type, item_id)
        account_name = existing_record["name"]
    else:
        is_new = True
        account_data = prepare_new_account_data(p_account, all_existing_accounts, local_type, item_id)
...
def sync_single_account_transactions(p_account_id: str, internal_account_id: str, from_date: datetime.datetime, api_key: str) -> int:
    """
    ...
    for p_txn in results:
        ...
        txn_exist = supabase.table("transactions").select("id").eq("provider_transaction_id", p_txn["id"]).execute()
...
def process_webhook_transaction(txn) -> bool:
    ...
    """
    res = supabase.table("accounts").select("id").eq("provider_account_id", txn.account_id).execute()
