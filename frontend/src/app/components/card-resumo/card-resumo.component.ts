...
@Component({
  ...
  template: `
    <div
      class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between h-32 relative overflow-hidden"
      [class.ring-1]="isHighlighted()"
      [class.ring-indigo-50]="isHighlighted()"
      [title]="tooltip()"
    >
      ...
      @if (isHighlighted()) {
        <div
          class="absolute -bottom-4 -right-4 w-24 h-24 bg-indigo-50 rounded-full opacity-50"
        ></div>
      }
    </div>
  `,
